...
pub fn feed_scanner_file(filename: &'static str, str: Option<String>) -> Vec<String> {
    match Path::exists(Path::new(filename)) { // guard incase test fails and is rerun
        true  => cleanup(filename),
        false => (),
    };
    match str {
        None => {
            match File::create_new(filename){ // create_new opens in read-write mode
                Ok(ok)   => ok,
                Err(err) => panic!("{err}"),
            };
        ...
        Some(str) => {
            let mut f = match File::create_new(filename){ // create_new opens in read-write mode
                Ok(ok)   => ok,
                Err(err) => panic!("{err}"),
            };
            // write to file
